...
public interface ExpenseTrasactionsRecordsRepository extends JpaRepository<ExpenseTrasactionsRecordsEntity, Integer> {
	...
	@Query(value = "SELECT etr.id as id, ROW_NUMBER() OVER (ORDER BY etr.date desc) as row_no, etr.date as date, etr.expenses_category_name as expense, etr.sub_category_name as type, etr.amount as amount, etr.payment_mode_name as payment_mode, etr.type_name as payment_mode_type, etr.by_whom as by_whom\r\n"
			+ "FROM expense_trasactions_records etr, expenses_category ec, expenses_sub_category esc, payment_mode pm, payment_mode_type pmt, by_whom pb where etr.expenses_category_name = ec.expenses_category_name \r\n"
